...
const updateUser = async (req, res) => {
    try {
        await Provider.findOneAndUpdate({email: req.body.email}, req.body, {new: true});
...
const deleteUser = async (req, res) => {
    try {
        ...
        if (user.role === 'Admin') {
            ...
            if (movies.length !== 0) {
                res.status(409).json({message: `User could not been deleted because there are movies associated with the user.`});
            } else {
